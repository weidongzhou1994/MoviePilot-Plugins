...
class SubscribeAssistant(_PluginBase):
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        """
        _tv_cat = [{"title": d, "value": d} for d in self.category.tv_categorys]
        _movie_cat = [{"title": d, "value": d} for d in self.category.movie_categorys]
        return [
            {
                'component': 'VForm',
                'content': [
                                                ...
                                                'content': [
                                                    {
                                                        'component': 'VSelect',
                                                        'props': {
                                                            'model': 'movie_exclude_type',
                                                            'label': '电影洗版排除二级分类',
                                                            'multiple': True,
                                                            'items': _movie_cat,
                                                            'hint': '选择需要排除自动洗版的电影的二级分类',
                                                            'persistent-hint': True
                                                        }
                                                    }
                                                ]
                                                ...
                                                'content': [
                                                    {
                                                        'component': 'VSelect',
                                                        'props': {
                                                            'model': 'tv_exclude_type',
                                                            'label': '电视节目洗版排除二级分类',
                                                            'multiple': True,
                                                            'items': _tv_cat,
                                                            'hint': '选择需要排除自动洗版的电视节目的二级分类',
                                                            'persistent-hint': True
                                                        }
                                                    }
                                                ]
    ...
    def process_best_version(self, subscribe_dict: dict, mediainfo: MediaInfo):
        ...
        if subscribe_type == MediaType.MOVIE:
            if str(mediainfo.category) in self._movie_exclude_type:
                logger.debug(f"电影类型二级分类{str(mediainfo.category)}被排除，跳过自动洗版处理")
                return
        
        if subscribe_type == MediaType.TV:
            if str(mediainfo.category) in self._tv_exclude_type:
                logger.debug(f"电视节目类型二级分类{str(mediainfo.category)}被排除，跳过自动洗版处理")
                return
